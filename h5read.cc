...
//       happen by querying Octave build info (which is since 4.4 no longer
//       exposed).  One cannot assume that the system Octave was built is the
//       same this package is built.  Anyway the info if Octave was build with
//       HDF5 does not matter for this package.
//
//       The runtime information is available with
#define HAVE_HDF5 1
  ...
  for (int i = 0; i < rank; i++)
    {
      ...
      unsigned int end = start(i) + _stride(i)*(_count(i)-1) + _block(i); // exclusive
  ...
  for (int i = 0; i < rank; i++)
    {
      ...
      unsigned int end = start(i) + _stride(i)*(count(i)-1) + _block(i); // exclusive
